...
import (
	...
	"kafka-exporter/go-zookeeper/zk"
)
...
// Brokers returns a map of all the brokers that make part of the
// Kafka cluster that is registered in Zookeeper.
func (kz *Kazoo) Broker(id string) (string, error) {
	root := fmt.Sprintf("%s/brokers/ids/"+id, kz.conf.Chroot)
	...
	return brokerNode.Host + ":" + strconv.Itoa(brokerNode.Port), nil
}
