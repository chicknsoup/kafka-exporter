...
func (c *Conn) connect() error {
	...
	for {
		...
		if err == nil {
			...
			if c.logInfo {
				log.Infoln("Connected to %s", c.Server())
			}
			return nil
		}

		log.Errorln("Failed to connect to %s: %+v", c.Server(), err)
	}
}
...
func (c *Conn) resendZkAuth(reauthReadyChan chan struct{}) {
	...
	if c.logInfo {
		log.Infoln("Re-submitting `%d` credentials after reconnect",
			len(c.creds))
	}

	for _, cred := range c.creds {
		if shouldCancel() {
			log.Infoln("Cancel rer-submitting credentials")
			return
		}
		...
		if err != nil {
			log.Errorln("Call to sendRequest failed during credential resubmit: %s", err)
...
func (c *Conn) loop() {
	for {
		...
		case err == ErrSessionExpired:
			log.Errorln("Authentication failed: %s", err)
		...
		case err == nil:
			if c.logInfo {
				log.Infoln("Authenticated: id=%d, timeout=%d", c.SessionID(), c.sessionTimeoutMs)
			}
